...
retry:
	for !poolsMatch {
		firstPool, err := nodes[0].Client.GetRawMempool(ctx)
		if err != nil {
			return err
		}
		...
		for _, node := range nodes[1:] {
			nodePool, err := node.Client.GetRawMempool(ctx)
			if err != nil {
				return err
			}
...
retry:
	for !blocksMatch {
		...
		for _, node := range nodes {
			blockHash, blockHeight, err := node.Client.GetBestBlock(ctx)
			if err != nil {
				return err
			}
...
func ConnectNode(from *Harness, to *Harness) error {
	peerInfo, err := from.Client.GetPeerInfo(ctx)
	if err != nil {
		return err
	}
	...
	if err := from.Client.AddNode(ctx, targetAddr, rpcclient.ANAdd); err != nil {
		return err
	}
